...
use manager::{Broadcast, SockJSManager};
use protocol::{CloseCode, Frame};
use session::{CloseReason, Message, Session};
...
impl<A> ActorContext for SockJSContext<A>
where
...
impl<A> AsyncContext<A> for SockJSContext<A>
where
    A: Session<Context = Self>,
{
    ...
    A: Session<Context = Self>,
{
    ...
    {
        self.sm
            .do_send(Broadcast::new(Frame::Message(message.into().0)));
    }
    ...
    fn add_to_buf(&mut self, msg: Frame) {
        ...
        if is_msg && msg.is_msg() {
            ...
            let vec = match item {
                BufItem::Message(m) => vec![m, msg.into_message()],
...
#[doc(hidden)]
impl<A> Future for SockJSContext<A>
where
    A: Session<Context = Self>,
{
    type Item = ();
    ...
    fn poll(&mut self) -> Poll<Self::Item, Self::Error> {
        let ctx: &mut SockJSContext<A> =
            unsafe { std::mem::transmute(self as &mut SockJSContext<A>) };
                            ...
                            match msg {
                                ...
                                SockJSChannel::Acquired(tx) => {
                                    while let Some(msg) = self.buf.pop_front() {
                                        match msg {
                                            BufItem::Message(msg) => {
                                                let _ = tx.unbounded_send(ChannelItem::Frame(
                                                    Frame::Message(msg),
                                                ));
                                            }
